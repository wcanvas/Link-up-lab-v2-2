...
/**
 * Generates custom entry objects for blocks.
 *
 * @return {Object} - An object where keys are output paths and values are absolute paths to the corresponding JS files.
 */
const customEntryBlocks = () => {
	return globSync( `./blocks/*/index.js` ).reduce(
		( files, filepath ) => {
			// Get the directory name of the index.js file.
			const dirPath = path.dirname( filepath );
			// Get the base name of the directory.
			const name = path.basename( dirPath );

			files[ `${ name }/index` ] = path.resolve(
				process.cwd(),
				`blocks/${ name }`,
				`index.js`
			);
...
/**
 * Generates custom entry objects for block view scripts.
 *
 * @return {Object} - An object where keys are output paths and values are absolute paths to the corresponding view.js files.
 */
const customEntryBlockViews = () => {
	return globSync( `./blocks/*/view.js` ).reduce(
		( files, filepath ) => {
			// Get the directory name of the view.js file.
			const dirPath = path.dirname( filepath );
			// Get the base name of the directory.
			const name = path.basename( dirPath );

			files[ `${ name }/view` ] = path.resolve(
				process.cwd(),
				`blocks/${ name }`,
				`view.js`
			);
